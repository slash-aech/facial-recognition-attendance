...
export default function TeacherReportDownload() {
  ...
  const generateReport = () => {
    ...
    if (!dateRange.start || !dateRange.end || !selectedAcademicYear || !selectedSemesterType) {
      setMessage('Please fill in all fields including dates and selections');
      return;
    }

    // Generate a dummy report URL with query params
    const report = `https://example.com/report?year=${selectedAcademicYear}&semester=${selectedSemesterType}&start=${dateRange.start}&end=${dateRange.end}`;
    setReportUrl(report);
